...
/// - Associated Values
/// 열거형의 각 케이스에는 관련 값을 정의할 수 있다.
/// Barcode 열거형은 upc케이스외, qrCode케이스를 가진다.
/// 각각의 케이스는 4개의 Int 튜플과 String 관련값을 가진다.
enum Barcode{
...
///관련값고 차이점은 관련값의 경우에는 관련값은 사용자가 변경할 수 있는 반면에,
///원시값은 고정되어 있다.
...
/// 열거형의 원시값이 Int타입이지만 각 케이스별로 원시값이 할당되어 있지 않다면,
/// 스위프트는 자동으로 0부터 케이스에 원시값을 할당한다.
/// venus = 2, earth = 3, mars = 4.....
enum PlanetOrder: Int{
    case mercury = 1,venus, earth, mars, jupiter, saturn, uranus, neptune
}

/// 열거형의 원시값이 String타입이지만 각 케이스별로 원시값이 할당되어 있지 않다면,
/// 스위프트는 자동으로 케이스명이랑 똑같은 문자열을 원시값으로 할당한다.
...
/// 다시 한번 말하지만, 열거형의 원시값(Raw Value)는 변경할 수 없으며 각 케이스마다 구분되는 값이다.
...
/// 재귀적 열거형은 어느 열거형의 케이스에 해당하는 관련값이 또다른 열거형 인스턴스인 경우이다.
/// 이때 관련값을 다른 열거형 인스턴스로 갖는 case 앞에 indirect 키워드를 붙혀줘야 한다.
