...
/// - Closure Expressions
/// 클로저의 매개변수는 가변 매개변수, in out 매개변수일 수는 있지만 기본 값을 가질 수 없다.
var names = ["Jack", "Anna", "Eva", "Alex", "Chris"]

/// sorted() 메소드에 인자로 전달된 클로저이다.
...
/// 스위프트의 인라인 클로저의 경우 대부분 스위프트의 타입추론에 의해 타입을 생략할 수 있으므로, 짧게 작성할 수 있다.
/// 하지만, 가독성에 문제가 생기는 경우에는 타입을 명시하는 것이 좋다.
/// 단축 인자 이름($0,$1.. )도 인라인 클로저의 경우 스위프트에서 제공해 주는 참조이다.

/*------------------------------------------*/

/// - Trailing Closures
/// 만약, 함수의 마지막 인자가 클로저(즉, 인라인 클로저)이고 그 클로저가 매우 길다면 후행 클로저 문법을 사용할 수 있다.
/// 후행 클로저를 사용하는 경우, 첫번째 인라인 클로저의 인자 레이블은 생략할 수 있다.
/// 또한 여러 개의 후행 클로저 문법을 사용할 수 있다.
...
/// 만약 이 경우와 같이 클로저 하나만 유일한 인자로 전달한다면, 이를 후행 클로저로 ()도 생략할 수 있다.
let reversedNames = names.sorted{$0 > $1}

/// 위와 같은 표현들은 인라인 클로저를 한 줄에 작성할 수 없을 경우에 효율적으로 사용할 수 있다.
/// Array에 map 메소드도 인자로 클로저를 받는데, 위의 표현으로 간편하게 줄일 수 있다.
/// map 메소드는 전달된 클로저에 각 원소가 전달되어, 동일한 순서의 배열을 새로 return한다.
...
/// 클로저를 하나만 받으므로 후행 클로저 문법을 사용해 ()를 생략하고 {}내에 클로저를 작성했고,
/// (parameter: type) -> return Type in CODE 꼴로 작성해야 하지만,
/// 매개변수의 타입은 스위프트의 타입추론 기능으로 생략하였다.
/// 따라서, numbers의 원소가 하나씩 map에 인자로 전달된 인라인 클로저의 매개변수로 전달되어 strings 배열이 생성되었다.

/// 함수의 인자에 클로저가 여러 개 있는 경우에도
...
/// - Autoclosures
/// 자동 클로저란 함수의 인라인 클로저로 전달되는 표현식을 래핑하기 위해 자동으로 생성되는 클로저이다.
...
/// 단순히 customerProvider라는 클로저를 정의한 것이다.
...
/// 자동 클로저를 사용하지 않고, 직접 클로저를 인자로 전달해 함수를 호출한 것이다.
...
/// 클로저의 바디, 즉 표현식만 전달했지만 자동클로저로 되어 {}가 자동으로 감싸진다.
serve(customer: customersInLine.remove(at: 0))
