...
///
/// 옵셔널 체이닝은 nil일 수도 있는 프로퍼티, 메소드, 서브스크립트에 접근하는 경우에 사용된다.
/// 옵셔널이 nil인 경우 프로퍼티, 메소드, 서브스크립트의 호출은 nil을 반환한다.
/// 만약, 옵셔널 체이닝의 어느 한 부분이라도 nil이면 전체 체인은 실패한다.

/// - Optional Chaining as an Alternative to Forced Unwrapping
/// 강제 언래핑과의 차이점은 강제 언래핑은 nil인 경우 런타임 에러가 발생하지만, 옵셔널 체이닝은 nil을 반환한다.
/// 옵셔널 체이닝의 return 타입은 옵셔널이다.
...
// john.residence?.numberOfRoom은 Int? 타입의 1
if let roomCount = john.residence?.numberOfRoom{
    print("john has \(roomCount)'s room")
}else{
    print("john has no room")
...
/// john.residence가 nil이므로, 호출 자체가 실패한다.
...
/// 신기하다.. 실패한 옵셔널 체이닝을 통한 할당 연산 자체가 Void? 타입인 nil이다.
...
/// - Note:
/// -   옵셔널 체이닝으로 조회하려는 타입이 옵셔널이 아닌 경우, 옵셔널 체이닝에 의해 옵셔널로 된다.
/// -   옵셔널 체이닝으로 조회하려는 타입이 옵셔널인 경우, 옵셔널 체이닝에 의해 옵셔널로 된다.
/// -   즉, 옵셔널 체이닝으로 인해 여러 겹의 옵셔널이 되는 것은 아니다. 그냥 옵셔널이다.
/// 만약 옵셔널 체이닝 중간에 nil이 있으면 무조건 nil을 반환한다.
/// street 프로퍼티가 옵셔널 체이닝을 통해 조회하려고 하는 값이고, String? 타입이다.
...
/// buildingIdentifier() 메소드는 String? 타입이다.
/// 옵셔널 체이닝을 통해 접근한buildingIdentifier()는 위에서 설명햇듯이 String? 타입이다.
...
/// buildingIdentifier() 메소드는 String? 타입이므로 옵셔널 체이닝을 통해 접근해야 한다.
