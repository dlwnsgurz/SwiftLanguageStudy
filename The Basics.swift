...
/// 다음과 같이 콤마(,)를 이용해 한 줄에 변수나 상수를 선언할 수 있다.
var A = 10, B = 20, C = 4.5

/// 변수나 상수의 이름뒤에 쿨론(:)을 붙여 타입 지정을 할 수 있다.
/// 이는 welcomeMessage 변수에 어느 문자열이건 저장될 수 있음을 의미한다.
var welcomeMessage: String
welcomeMessage = "Hello"

/// 콤마(,) 를 통해 여러 개의 상수나 변수에 대해 타입 지정을 할 수 있다.
var red, green, blue: Double

/// 변수나 상수의 이름은 숫자로 시작할 수 없다.
...
/// print(_seperator: terminator:) 전역함수를 통해 변수나 상수에 저장된 값을 확인할 수 있다.
/// 만약 개행을 하고 싶지 않다면 terminator = "" 를 전달함으로써 개행을 안할 수 있다.
/// 이는 default 값으로 terminator 인자가 개행 값을 갖기 때문이다.
...
/// - Semicolons
/// 스위프트는 세미콜론을 사용할 필요가 없다.
/// 하지만, 둘 이상의 명렁어를 한 줄에 쓰기 위해선 세미콜론을 사용해야한다.
let cat = "🐈"; print(cat)
...
/// 다음 프로퍼티를 정수 타입의 최대값, 최소값을 구할 수 있다.
let maxValue = UInt8.max
let minValue = UInt8.min

/// - Note :
/// 일반적으로 음수일 가능성이 없는 정수값을 저장하는 경우에도 UInt 자료형보다는
/// Int 자료형을 사용하는 것이 일반적이다. 왜냐하면, 스위프트의 타입 추론 기능은 정수를 Int로 추론하기 때문이다.

/*------------------------------------------*/

/// - Floating-Point Numbers
/// 스위프트에서는 2개의 부동소수점 타입이 사용된다.
/// 64비트의 Double과 32비트의 Float이다.
/// Double은 소수점 15번째 자리까지 유효성이 보장되고, Float은 6번째 자리까지 유효성이 보장된다.
/// 스위프트에서 일반적으로 Double이 선호된다.
...
/// 1. 일반적 리터럴 방식(소수점 얖 옆에 반드시 숫자가 있어야한다.)
/// 2. 10진수 지수 표현방식(e, E)
/// 3. 16진수 지수 표현방식(p, P)
...
/// 따라서 특수한 경우에만 사용하는 것이 적절하며, 이를 통해 코드의 의도를 쉽게 파악할 수 있다.

/// 타입() 생성자를 통해 형변환이 가능하며, 이는 생성자의 매개변수에 여러 타입이 미리 정의되어 있음을 알 수 있다.
...
/// - Booleans
/// 변수나 상수를 불리언 타입의 값(true,false)로 초기화할 떄
/// 타입을 명시할 필요는 없다.
...
/// 튜플에서 개별의 값을 분해할 때 크게 3가지 방법이 있다.
let http404Error = (404, "Not Found")
// http404Error is of type (Int, String), and equals (404, "Not Found")

/// 첫번째 방법, 튜플 요소를 별도의 상수나 변수로 분해할 수 있다.
let (firstElement, SecondElement) = http200Error

/// 두번째 방법, 언더케이스(_)로 불필요한 값은 분해하지 않을 수 있다.
let (errorCode, descrption) = http404Error
let (errorCodeRetrived, _) = http404Error

/// 세번째 방법, 인덱스를 통한 접근 방법
let errorNumber = http404Error.0
let errorMessage = http404Error.1

/// 또한, 튜플 선언 시 요소의 이름을 지어줄 수 있다.
/// 튜플 요소의 이름으로 접근도 가능하다.
...
/// 옵셔널은 값을 가지고 있거나, 값이 없음을 내포한다.

/// convertNumber의 타입은 Int가 아닌 Int?이다.
/// 문자열 "123" 은 Int형 리터럴인 123으로 변환이 가능하지만 "hello" 같은 경우에는
/// 정수로 바꿀 수 없다. 따라서 nil이 들어갈 수 있도록 Int?타입이 된다.
let possibleNumber = "123"
let convertedNumber = Int(possibleNumber)

/// 옵셔널이 아닌 타입에는 nil이 들어갈 수 없다.
/// 만약 초기값 없이 옵셔널 타입으로만 선언한다면 자동으로 nil이 저장된다.
/// - Note: nil은 nil이라는 값이 아니라 "값이 없음을" 의미한다.
...
/// 두번째는 옵셔널 바인딩으로, if let 구문이나 while let 구문을 이용한다.
...
/// 콤마(,)를 통해 구분한다.
/// 또한 옵셔널이 하나라도 nil이거나 조건이 하나라도 false인 경우 탈출한다.
...
/// 암시적 추출 옵셔널은 옵셔널 타입이지만, nil이 되지 않을 확실한 상황인 경우에 사용한다.
...
let optionalString = assumedString // String?

/// 하지만 일반 옵셔널로 처리할 수 없는 경우에는 내부 값으로 인해 변경된다.
let valueString: String = assumedString // String
...
/// 에러가 발생할 수 있는 함수는 throws 키워드를 이용한다.
/// 에러가 발생할 수 있는 표현식은 앞에 try 키워드를 붙여야한다.
...
/// makeASandiwich()를 실행하고 에러가 발생하면 catch절로 에러가 반드시 전달된다.
/// 만약 에러가 발생하지 않았다면, eatSandwich()가 실행되고, do 구문을 탈출한다.
do{
    try makeASandwich()
    eatSandwich()
