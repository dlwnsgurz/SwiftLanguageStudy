...
/// 또한, 스위프트에서는 프로토콜을 따르는지 따르지 않는지 타입 캐스팅을 통해 확인할 수 있다.
...
/// 이 인스턴스들의 공통점은 MediaItem이라는 슈퍼 클래스를 상속한다는 점이다.
/// 따라서 스위프트는 이 상수 배열의 타입을 [MediaItem]으로 추론한다.
/// 만약 이 배열을 iterate 한다면, MediaItem 타입의 인스턴스를 얻게 된다.
/// 이를 Move, Song 인스턴스로 다루기 위해서는 타입을 확인하거나, 다운 캐스팅하는 과정이 필요하다.
let library = [
...
/// - Checking Type
/// 만약, 특정 클래스 인스턴스가 서브 클래스의 인스턴스인지 확인하려면 is 연산자를 사용하면 된다.
...
/// - Downcasting
/// 어느 클래스의 인스턴스를 서브 클래스의 인스턴스로 처리하려면 타입 캐스팅 연산자인 as!나 as?를 사용하면 된다.
/// as?는 옵셔널을 반환하며, 만약 하위 클래스가 아닌 경우에 nil을 반환한다.
/// as!는 타입 인스턴스를 반환하며, 만약 하위 클래스가 아닌 경우에 런타임 에러가 발생한다.
/// 아래의 코드의 경우 as?를 사용하였다, 왜냐하면 각 원소를 iterate할 때 Song 타입인지, Movie 타입인지 확신할 수 없기 때문이다.
/// as? 는 옵셔널 타입을 반환하므로, 옵셔널 바인딩을 이용하였다.
for item in library{
...
/// Any 타입의 값들을 처리하는데는 switch 구문이 유용하게 사용된다.
var things: [Any] = []
things.append(0) // Int
