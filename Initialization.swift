...
/// 이를 위해 생성자 내에서 값을 할당하던가, 저장 프로퍼티의 기본값이 할당 되어 있어야 한다.
/// 만약, 생성자를 통해 값을 할당하는 경우, 프로퍼티 감시자가 시행되지 않는다.
/// 또한, 만약 어떤 저장 프로퍼티가 항상 일정한 기본값을 갖는 경우라면, 생성자를 통해
...
/// 구조체는 하나의 생성자도 정의되어 있지 않다면 멤버와이즈 생성자를 제공한다.
/// 모든 저장 프로퍼티를 인자로 전달할 수 있으며, 기본 값이 제공되어 있는 경우에는 심지어 생략도 할 수 있다.
struct Size {
...
/// 구조체와 열거형에서 사용자 정의 생성자를 정의하면, 멤버와이즈 생성자와, 기본 생성자를 사용할 수 없다.
/// 만약, 사용하고 싶다면 extension을 이용해야 한다.
struct Size {
    var width = 0.0, height = 0.0
}
struct Point {
...
/// 클래스의 모든 프로퍼티는 초기화중에 반드시 초기 값이 할당되어야 한다.
/// 클래스내에서 초기자는 크게 2가지이다.
/// 첫번째는 지정 초기자로 프로퍼티를 초기화하는 역할을 한다. 즉 반드시 한 개 이상 존재해야 한다.
...
/// 첫번째는 객체를 위한 메모리가 할당되고, 최상위 클래스에 도달할 때까지 동일 계층에 모든 저장 프로퍼티가 하위 클래스부터 쭉 초기화 되어 올라간다.
...
/// 2. 상속받은 프로퍼티에 값을 할당하기전에 반드시 지정생성자로 상위 클래스의 지정 생성자를 호출해 초기화를 해야한다. (초기화를 하기전에 하위 클래스에서 상속받은 프로퍼티에 값을 할당할 수 없다.)
/// 3. 편의 생성자는 프로퍼티에 값을 할당하기전에 반드시 생성자를 호출해, 초기화를 진행해야한다.
/// 4. 이니셜라이저는 모든 프로퍼티가 초기화 되기 전까지는 self의 메소드, 프로퍼티등에 접근이 불가능하다.
///
/// 상위 클래스의 편의 생성자는 하위 클래스에서 호출될 수 없다. 따라서 하위클래스에서 상위 클래스의 초기자는 override를 붙이지 않아도 된다.
...
/// 1. 만약 하위 클래스에서 지정 초기자가 정의되지 않는다면, 상위 클래스의 지정 초기자가 자동 상속된다.
...
/// 실패 가능한 이니셜라이저도 정의할 수 있다.
/// init뒤애 ?를 붙여 init?(parameter)로 정의한다. 실패 가능한 생성자와 실패 불가능한 생성자를 동시에 정의할 수 없다.
/// 실패 가능한 생성자는 nil을 return 하도록 정의하면 된다. 그렇다고 해서 그냥 생성자가 return 값이 있는 것은 아니다.
...
/// 구조체, 열거형, 클래스에서 실패 가능한 이니셜라이저는 실패 가능한 이니셜라이저에게 초기화를 위임할 수 있으며,
...
/// 상위 클래스의 실패 가능한 생성자를 하위 클래스에서 실패 불가능한 생성자로 오버라이딩 할 수 있다.
/// 하지만, 그 반대는 성립하지 않는다.
class Document{
    var name: String?
...
/// 상위 클래스의 실패 가능한 생성자를 하위 클래스에서 실패하지 않는 초기자로 오버라이드 했을떄
...
/// 서브 클래스에서 강제로 구현해야 하는 상위 클래스의 이니셜라이저를 정의할 때, required 키워드를 붙이면 된다.
/// 서브 클래스에서는 그 이니셜라이저를 구현할 때 override 키워드를 붙힐 필요 없이 required 키워드만 붙이면 된다.
/// 만약 required 생성자를 자동 상속 받을 수 있는 조건이 충족되면, 서브 클래스에서 반드시 구현할 필요는 없다.
class SomeClass{
    required init() {}
}
...
/// 클로저의 {} 뒤에 ()는 그 즉시 실행을 의미하기때문에, 다른 프로퍼티들이 초기화 되기 전에 클로저가 실행된다.
/// 따라서, 클로저 내부에서 인스턴스의 프로퍼티와 메소드에 접근이 불가능하다.
...
struct Chessboard {
    let boardColors: [Bool] = {
        ...
        return temporaryBoard // Bool 타입이다.
