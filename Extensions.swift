...
extension SomeType: SomeProtocol, AnotherProtocol{
    // 프로토콜이 요구하는 사항을 구현해야함.
}

/*------------------------------------------*/

/// - Computed Properties
/// extension을 이용해 연산 인스턴스 프로퍼티와 타입 연산 프로퍼티를 추가할 수 있다.
/// 하지만, 저장 프로퍼티나 기존 저장 프로퍼티에 관찰자를 추가할 수 없다.
extension Double{
...
/// - Initializers
/// 구조체나, 열거형, 클래스에서는 extension을 이용해 새로운 초기자를 추가할 수 있다.
...
/// 하지만, 확장을 통해 구현한 이니셜라이저내부에서는 기존 타입에 정의되어있는 이니셜라이저를 호출하기 전까지 self에 접근이 불가능하다.
/// 따라서, 반드시 기존에 정의된 이니셜라이저를 호출해야 한다.
struct Size{
...
/// extension에서 추가한 인스턴스 메소드의 경우 self나 프로퍼티를 수정하는 구조체와 열거형 타입에는
/// 반드시, mutating 키워드를 붙여야 한다.
extension Int{
...
/// 위의 중첩된 열거형과 읽기 전용 연산 인스턴스 프로퍼티를 Int 타입의 인스턴스에서 사용할 수 있다.
